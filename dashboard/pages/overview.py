...
def build_main_curve():
    ...
    # Critical-point band (paper: 14 ± 3 yrs from 2026 → 2037–2043, central 2040)
    fig.add_vrect(
        x0=2040, x1=2047,
...
layout = html.Div([
    help_section("Overview", [
        ...
        "REFERENCE LOGISTIC (faint dotted grey): For visual contrast, the chart also plots a standard constant-r logistic curve calibrated to the same observed segment (1990 → 2025). This reference reaches the critical threshold (~95) in 2051. The Gratacós-Botto super-exponential reaches the same threshold in 2047 — a 4-year advance of the critical-ineffectiveness point, which is the central forecasting claim of the paper.",
        "UNCERTAINTY BANDS (±3 YEARS): The shaded blue envelope around the primary curve is generated by laterally shifting the trajectory by ±3 years, matching the paper's stated uncertainty (párr. 5: 'fourteen years (±3)'). It is not a statistical confidence interval. Interpret it as: the critical threshold could realistically fall up to 3 years earlier or later than the central forecast. The band tightens at the asymptote because all trajectories converge to the same K=100.",
    ...
    html.Div([
        chart_title_with_info(
            ...
            "Primary curve: super-exponential model from the Gratacós-Botto paper (UPDATE v.A-29) with rate growing linearly in time. Faint dotted grey: reference standard logistic (constant r) for comparison. Red-shaded zone: projected critical-point window (2040–2047).",
